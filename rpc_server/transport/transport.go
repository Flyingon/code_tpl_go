...
// Checker 验包，预先分配大内存buf，校验数据并返回包大小，很通用，但是缺点是需要预先分配内存，对性能影响较大
...
// RegisterChecker register checker
func RegisterChecker(name string, c Checker) {
	if c == nil {
		panic("transport: register checker nil")
	}
	if name == "" {
		panic("transport: register checker name empty")
	}
	muxCheckers.Lock()
	checkers[name] = c
	muxCheckers.Unlock()
...
// GetChecker get checker by name
